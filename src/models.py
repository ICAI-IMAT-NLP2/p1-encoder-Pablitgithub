...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([
            AttentionHead(d_model=d_model, d_k=d_head, d_q=d_head, d_v=d_head)
            for _ in range(num_attention_heads)
        ])

        # Proyección final tras concatenar (B, T, h*d_head) = (B, T, d_model)
    ...
    def forward(self, hidden_state):
        ...
        """
        head_outputs = [head(hidden_state) for head in self.heads]  # cada uno (B, T, d_head)
        x = torch.cat(head_outputs, dim=-1)  # (B, T, h*d_head) == (B, T, d_model)

        x = self.output_linear(x)  # (B, T, d_model)
        return x
...
class Embeddings(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        position_ids = torch.arange(seq_length, device=input_ids.device).unsqueeze(0).expand(input_ids.size(0), seq_length)
